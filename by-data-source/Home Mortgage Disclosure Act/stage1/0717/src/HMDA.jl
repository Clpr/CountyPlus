...
const ALL_RACES = (
    ...
    "black", # code: 3 
    "asian", # code: 2 
    "valid", # applicable if any known race identified
)
...
Base.@kwdef mutable struct EnvVars
    ...
    data_dir::String = "../HMDA data - raw/HMDA 07-17 allrecords/"

    # data as zip archive
    datafiles_zip::Dict{Int,String} = Dict{Int,String}(
        2007 => "hmda_2007_nationwide_all-records_labels.zip",
        2008 => "hmda_2008_nationwide_all-records_labels.zip",
        2009 => "hmda_2009_nationwide_all-records_labels.zip",
        2010 => "hmda_2010_nationwide_all-records_labels.zip",
        2011 => "hmda_2011_nationwide_all-records_labels.zip",
        2012 => "hmda_2012_nationwide_all-records_labels.zip",
        2013 => "hmda_2013_nationwide_all-records_labels.zip",
        2014 => "hmda_2014_nationwide_all-records_labels.zip",
        2015 => "hmda_2015_nationwide_all-records_labels.zip",
        2016 => "hmda_2016_nationwide_all-records_labels.zip",
        2017 => "hmda_2017_nationwide_all-records_labels.zip",
    )

    # data file after inflation
    datafiles_unzipped::Dict{Int,String} = Dict{Int,String}(
        2007 => "hmda_2007_nationwide_all-records_labels.csv",
        2008 => "hmda_2008_nationwide_all-records_labels.csv",
        2009 => "hmda_2009_nationwide_all-records_labels.csv",
        2010 => "hmda_2010_nationwide_all-records_labels.csv",
        2011 => "hmda_2011_nationwide_all-records_labels.csv",
        2012 => "hmda_2012_nationwide_all-records_labels.csv",
        2013 => "hmda_2013_nationwide_all-records_labels.csv",
        2014 => "hmda_2014_nationwide_all-records_labels.csv",
        2015 => "hmda_2015_nationwide_all-records_labels.csv",
        2016 => "hmda_2016_nationwide_all-records_labels.csv",
        2017 => "hmda_2017_nationwide_all-records_labels.csv",
    )

    # master file for merging with other year-county data
    master_file::String = "0717/options/fips.csv"
    ...
    process_specs::String = "0717/options/process_specs.csv"

    # tells which aggregators/summarizers should be defined during aggregation
    # unified across year for code re-usability
    aggregator_file::String = "0717/options/aggregators.csv"

    # variable list in final cleaned data
    datadict_final::String = "0717/options/datadict_final.csv"
...
function Base.push!(
    ...
    if !haskey(ya.dat,(fips,vname))
        # error(@sprintf("(%d,%s) not exist",fips,vname))
...
function pushrow!(
    ...
    begin
        ...
        if isnothing(record["fips_state"]) || isnothing(record["fips_county"])
            return nothing
        end
    ...
    fips::Int = record["fips_state"] * 1000 + record["fips_county"]
    (fips ∈ env.all_fips) || (return nothing)
    ...
    begin
        ...
        for i in "race" .* string.(1:5)
            ...
            flags["asian"] |= record[i] == 2
            flags["valid"] |= record[i] ∈ (1,2,3,4,5)
        end
